...
namespace LightningReview.ExtensionFramework
{
	...
	public interface IReviewWindow
	{
        ...
        /// 指摘を追加するアウトラインのパス。
        /// "/ドキュメント名/アウトラインノード名"のように指定します。
        /// 一致するパスが存在しない場合、エラーを返します。
        /// </param>
        /// <param name="description">
        /// クイックレビューウインドウの指摘内容に記入する文字列。
        /// 空文字の場合、クイックレビューウインドウの指摘内容は空です。
        /// </param>
        /// <param name="metaDatas">
        /// メタデータ。
        /// クイックレビューウインドウで追加する指摘にメタデータを設定します。
        /// </param>
        void ShowQuickReviewWindow(string outlinePath, string description = "", IDictionary<string, string> metaDatas = null);
